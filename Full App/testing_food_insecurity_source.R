...
food_insecurity_ui <- fluidPage(
  ...
  ### 
  tags$style('#food_sufficiency_ui {   height: 900px;   overflow: auto;}'),
  tags$script(
    '
    Shiny.addCustomMessageHandler("scrollCallback",
    function(msg) {
    console.log("aCMH" + msg)
    var objDiv = document.getElementById("food_sufficiency_ui");
...
fi_server <- function(input, output, session) {
  
  ###
  output$food_sufficiency_ui <- renderUI({
  ...
  shinyjs::hide("food_sufficiency_ui")
  ...
  shinyjs::onclick("fi_overall", 
                   function() {
                     ...
                     shinyjs::show("food_sufficiency_ui")
